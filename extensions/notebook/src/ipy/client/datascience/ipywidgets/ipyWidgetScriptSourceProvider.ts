...
import { ILocalResourceUriConverter, INotebook, IJupyterConnection } from '../types';
	...
	private rebuildProviders() {
		...
		if (this.notebook.connection && this.notebook.connection.localLaunch) {
			...
			if (this.notebook.connection) {
				scriptProviders.push(new RemoteWidgetScriptSourceProvider((<IJupyterConnection>this.notebook.connection), this.httpClient));
			}
		}
